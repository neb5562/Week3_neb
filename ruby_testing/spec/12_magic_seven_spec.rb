...
describe MagicSeven do
  ...
  describe '#subtract_random_number' do
    context "random number is 10"
    subject(:rgame) { described_class.new(1) }

    it "will return 7" do
      previous_step = 8 # Arrange
      result = rgame.subtract_random_number(previous_step) # Act
  ...
  describe '#play' do
    context 'when the random number is 25' do
      subject(:rgame) { described_class.new(25) }
      # remove the 'x' before running this test
      it 'will return 7' do
        expect(rgame.play).to eq(7)
      end
    end

    context 'when the random number is 69' do
      subject(:rgame) { described_class.new(69) }
      # remove the 'x' before running this test
      it 'will return 7' do
        expect(rgame.play).to eq(7)
      end
    ...
    context 'when the random number is 420' do
      subject(:rgame) { described_class.new(420) }
      # remove the 'x' before running this test
      it 'will return 7' do
        expect(rgame.play).to eq(7)
      end
